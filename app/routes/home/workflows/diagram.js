...
export default Route.extend({
    ...
    return this.get("store")
      ...
      .then(workflow => {
        ...
        return RSVP.hash({
          ...
          groupedOperations: groupBy(
            this.store.query("operation", queryParams),
            "categories"
          ),
          images: [
            { id: 0, name: "img0.jpg" },
  ...
  setupController(controller, model) {
    ...
    model.workflow.get("tasks").forEach(task => {
      task.operation = model.operations
        .findBy("id", String(task.operation.id))
