    ...
    if (validate(step)) {
      ...
      switch (step) {
        ...
        case 'run':
          ...
          // Draw svg
          _var.wrap = _var.container.d3.selectAll(`svg.chart-${_var._id}`).data(["chart-svg-area"], d => d);
          _var.wrap.exit().remove();
          _var.wrap = _var.wrap.enter().append("svg").attr('class', `area-chart chart-${_var._id}`).merge(_var.wrap); // svg
